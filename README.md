...
[![CI](https://github.com/romanmahachkala05/CatsListKMP/actions/workflows/ci.yml/badge.svg?branch=dev)](https://github.com/romanmahachkala05/CatsListKMP/actions/workflows/ci.yml)

An endless feed of cats from [TheCatAPI](https://developers.thecatapi.com/), with
favorites kept in Room and one-tap image download — one Compose Multiplatform app on
**Android, desktop (Windows, macOS, Linux) and iOS**, every screen shared.
...
pull request at a time, with the build green at every commit — and then moved,
module by module, to Kotlin Multiplatform.
...
| Platform | How |
| --- | --- |
| Android | APK from the [latest release](https://github.com/romanmahachkala05/CatsListKMP/releases/latest) |
| Windows, macOS, Linux | MSI, DMG or DEB from the [latest release](https://github.com/romanmahachkala05/CatsListKMP/releases/latest) — unsigned, so SmartScreen and Gatekeeper warn on first launch |
| iOS | Build from source (below). Without a paid Apple Developer account there is no TestFlight or App Store build |

---

## Architecture

Eleven Gradle modules and an Xcode project, Clean Architecture, one direction of
dependency:

```
:app, :desktopApp, iosApp/  ──►  :shared  ──►  :feature:feed, :feature:favorites  ──►
                  :core:model, :core:domain, :core:data, :core:ui, :core:designsystem
```

Everything below the three entry points is Kotlin Multiplatform, building for
Android, the JVM (desktop) and iOS — the screens included, in Compose
Multiplatform. `:app`, `:desktopApp` and `iosApp/` are an Activity, a window and a
SwiftUI view around the same `CatsApp()`, which adapts to the window: one column
under a bottom bar on a phone, a grid of two to four beside a navigation rail on
anything wider. `:core:model` and `:core:domain` know nothing about any platform;
`:core:data` implements the repository a use case declares and splits on four
seams — the database path, the HTTP engine, image download and the network
monitor.
...
| | |
| --- | --- |
| UI | Compose Multiplatform, Material 3, Coil |
| Navigation | Navigation 3 (`NavDisplay`, typed `NavKey`s) |
| DI | Koin |
| Async | Coroutines, Flow |
| Network | Ktor — OkHttp on Android and desktop, URLSession on iOS; one engine per app, shared with Coil |
| Multiplatform | Kotlin Multiplatform + Compose Multiplatform: Android, desktop and iOS |
...
| Tests | JUnit4, Truth, `kotlin.test` on iOS, Compose UI tests on Android and desktop, `kotlinx-coroutines-test`, hand-written fakes |

## Tests

**111 unit tests on the JVM, 6 on an iOS simulator, 38 instrumented.** No mocking library — every test double is a
...
[`docs/DECISIONS.md`](docs/DECISIONS.md): 41 decision records with the rejected
alternative and the consequences, including four superseded by a later one and
three amended. A sample:
...
- **[ADR-0039](docs/DECISIONS.md#adr-0039)** — iOS joined as a target, not a
  rewrite: the two JVM-isms left in common code went, and the very first iOS test
  run caught a crash that would have shipped — `NSLog` segfaulting on a Kotlin
  string passed through C varargs.
...
```bash
git clone https://github.com/romanmahachkala05/CatsListKMP.git
cd CatsListKMP
...
**iOS** needs a Mac with Xcode: open `iosApp/iosApp.xcodeproj` and run the `iosApp`
scheme — its first build phase builds the shared Kotlin framework. A simulator needs
nothing else. For your own iPhone, set your team in Xcode's Signing tab (or
`TEAM_ID` in `iosApp/Configuration/Config.local.xcconfig`); a free Apple ID works,
and the install lasts seven days.
...
([ADR-0034](docs/DECISIONS.md#adr-0034)) — and is a required check on `dev`. A
macOS job beside it builds the iOS targets, runs their tests on a simulator and
builds the Xcode project; on a Mac, `verify` itself runs the iOS tests too.
...
Multiplatform-specific: most unit tests use JUnit and Truth, so they run on the
JVM targets only — iOS runs its own six, not the whole suite. Coil is held at
3.4.0 until the project moves to Kotlin 2.4, because newer Coil releases ship iOS
libraries this Kotlin cannot read ([ADR-0039](docs/DECISIONS.md#adr-0039)). iOS is
build-from-source only, and the desktop installers are unsigned
([ADR-0041](docs/DECISIONS.md#adr-0041)).
